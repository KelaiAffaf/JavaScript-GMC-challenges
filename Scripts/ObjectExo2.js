...
function rateProduct(products, productId, userId, rate) {
  ...
  const userRated = productExists.ratings.find((r) => r.userId === userId);
  if (userRated) {
    userRated.rate = rate;
    return {
      success: true,
...
console.log(rateProduct(products, "aegfal", "fg12cy", 5));
console.log(products);

// rate product using for loop
function rateProduct2(products, productId, userId, rate) {
    ...
    if (products[i]._id === productId) {
      ...
      for (let j = 0; j < users.length; j++) {
        if (users[j]._id === userId) {
          const user = users[j];
          // check if this user already rated the product we update it
          for (let k = 0; k < product.ratings.length; k++) {
            if (product.ratings[k].userId === userId) {
              product.ratings[k].rate = rate;
              return {
                success: true,
                product: product,
                user: user,
      ...
      // if user doesn't exist, add the rating
...
console.log(rateProduct2(products, "aegfal", "fg12cy", 5));
console.log(rateProduct2(products, "aegfal", "fg12cy", 3));
